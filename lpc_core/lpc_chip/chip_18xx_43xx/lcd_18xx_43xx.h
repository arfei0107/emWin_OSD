...
typedef enum {
	/*
	//Modify by Stan,
	//   since it can just use bits/pixel number for filling into register
	//   so, modify LCD_CONFIG_T:BPP datatype from uint8_t to enum LCD_BPP_OPT_T
	...
	*/
	...
	LCD_BPP_4BPP          = 2,
	LCD_BPP_8BPP          = 3,
	LCD_BPP_16BPP_1_5_5_5 = 4,
	LCD_BPP_24BPP         = 5,
	LCD_BPP_16BPP_5_6_5   = 6,
	LCD_BPP_12BPP         = 7
} LCD_BPP_OPT_T;
...
typedef struct {
	...
	//uint8_t  BPP;	/*!< Maximum bits per pixel the display supports */,    //Mask by Stan for modifying data type
	LCD_BPP_OPT_T  BPP;	/*!< Maximum bits per pixel the display supports */ //change data type from uint8_t to enum LCD_BPP_OPT_T
	//Modify by Stan,
	uint16_t pcd; /*!< Add by Stan, panel clock divisor */
...
//Add by Stan for LCD_DMA FIFO watermark control
/**
 * @brief	Set LCD_DMA is requested by FIFO watermark >= 4 
 * @param	pLCD	: The base of LCD peripheral on the chip
 * @return	None
 */
STATIC INLINE void Chip_LCD_DMA_WATERMARK_4(LPC_LCD_T *pLCD)
{
	pLCD->CTRL &= ~(0x1 << 16);
}
...
//Add by Stan for LCD_DMA FIFO watermark control
/**
 * @brief	Set LCD_DMA is requested by FIFO watermark >= 8 
 * @param	pLCD	: The base of LCD peripheral on the chip
 * @return	None
 */
STATIC INLINE void Chip_LCD_DMA_WATERMARK_8(LPC_LCD_T *pLCD)
{
	pLCD->CTRL |= (0x1 << 16);
}
...
/**
 * @brief	Get Controller mask Interrupt status
 * @param	pLCD	: The base of LCD peripheral on the chip
 * @return	ints	: OR'ed interrupt bits
  */
STATIC INLINE uint32_t Chip_LCD_IntStatus(LPC_LCD_T *pLCD)
{
	return (uint32_t) pLCD->INTSTAT;
}

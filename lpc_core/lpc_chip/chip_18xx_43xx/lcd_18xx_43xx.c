...
void Chip_LCD_Init(LPC_LCD_T *pLCD, LCD_CONFIG_T *LCD_ConfigStruct)
{
	...
	/* panel clock divisor */
	pcd = (uint32_t)LCD_ConfigStruct->pcd; //Modify by Stan, add pcd into LCD_CONFIG_T
	if(pcd < 3) pcd = 0;             //Modify by Stan, divisor = 2 + pcd, so need to subtract 2
	else if(pcd > 1023) pcd = 1023;  //Modify by Stan, pcd max value is 1023
	else pcd -=2;                    //Modify by Stan, divisor = 2 + pcd, so need to subtract 2
	
	pcd &= 0x3FF;
	regValue |=  (((pcd >> 5)&0x1F) << 27) | ((pcd) & 0x1F);
	pLCD->POL = regValue;

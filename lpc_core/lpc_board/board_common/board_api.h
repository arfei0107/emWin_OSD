...
/**
 * @brief	Setup system clocking using external XTAL oscillator as main clock source
 * @return	None
 * @note	XTAL is used as input source for main system PLL. For applications using USB
 * in full-speed or high-speed mode should use this clocking scheme.
 * starting.
 */
void Board_SetupXtalClocking(void);

/**
 * @brief	Setup system clocking using IRC as main clock source
 * @return	None
 * @note	IRC is used as input source for main system PLL. For applications which 
 * need higher precision clocking should use external crystal and call Board_SetupXtalClocking().
 * starting.
 */

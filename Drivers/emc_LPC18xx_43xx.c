...
void EMC_Init( void )
{
  ...
  LPC_SCU_CLK(1) = 0 + EMC_IO;   // ... but all other pins need to be set as well,
  LPC_SCU_CLK(2) = 0 + EMC_IO;   // in order to get a correct CLK0 signal.
  LPC_SCU_CLK(3) = 0 + EMC_IO;
	
	DEBUGSTR("EMC_Config_Pinmux() is done\r\n");

	/*
  #ifdef CLKOUT_HALF
    LPC_CCU1->CLKCCU[CLK_MX_EMC_DIV].CFG |= (1<<0) | (1<<5);		// Turn on clock/2
    LPC_CREG->CREG6 |= (1<<16);	                        // EMC divided by 2
    LPC_CCU1->CLKCCU[CLK_MX_EMC_DIV].CFG |= (1<<0);		            // Turn on clock
	  DEBUGSTR("CLKOUT_HALF is done\r\n");
  #endif
	*/
    ...
    // Initialise the control signals for static memories	
    EMC_Config_Static();
		DEBUGSTR("EMC_Config_Static() is done\r\n");
    #ifndef EXT_DYN_MEM
  ...
  // The setup for dynamic memories (SDRAM)
    ...
    EMC_Init_SRDRAM(SDRAM_BASE, PART_WIDTH, PART_SIZE, EXT_WIDTH, COL_ADDR_BITS);
		DEBUGSTR("EMC_Init_SRDRAM() is done\r\n");
  #endif
...
/****************************************************************************************
* Set up the address/data pins for external memory interface in LPC18xx/LP43xx
*
...
// EMC_OUT	   (PUP_DISABLE | SLEWRATE_FAST | FILTER_DISABLE) 
// EMC_IO	   (PUP_DISABLE | SLEWRATE_FAST | INBUF_ENABLE | FILTER_DISABLE)
...
void EMC_Config_Static(void)
{
  ...
  // @120MHz there should be 8 waitstates for the 70ns flash, apparently it works with 7
  ...
  LPC_EMC->STATICWAITOEN0 = 0;              // CS0: WAITOEN = 0
    ...
    LPC_EMC->STATICWAITRD2 = 6;               // CS2: WAITRD = 7
  #endif
...
#ifdef AOCI_BOARD
  #define DOUT_DELAY      0
  #define CLK0_DELAY      0x66	  
  #define CLKE0_DELAY     0x66	  
  #define RAS_DELAY       0
  #define CAS_DELAY       0
  #define WE_DELAY        0
  #define DYCS0_DELAY     0
...
/****************************************************************************************
* Configure the delays for the SDRAM. These settings could be individual for each board.
* (see defines above)
*
...
void initEmiDelays(void)
{
    ...
    // CLK & CLKE0 delay
    *(uint32_t*)(LPC_SCU_BASE + 0xD00) = ((CLK0_DELAY << 8) | (CLKE0_DELAY << 0) );
...
void EMC_Init_SRDRAM(uint32_t u32BaseAddr, uint32_t u32Width, uint32_t u32Size, uint32_t u32DataBus, uint32_t u32ColAddrBits)
{
	...
	delayBase1us = SystemCoreClock / DELAY_1usFreq;

	// eventually adjust the CCU delays for EMI (default to zero)
	initEmiDelays();
	DEBUGSTR("initEmiDelays() is done\r\n");
	...
	//LPC_EMC->CONFIG 			= 0;
	DEBUGSTR("LPC_EMC->CONTROL = 0x00000001;\r\n");
//#if (PLATFORM == HITEX_A2_BOARD)
...
#if defined AOCI_BOARD
    /*
	LPC_EMC->DYNAMICCONFIG0 	= ((u32Width << 7) | (u32Size << 9) | (u32DataBus << 14)); // Selects the configuration information for dynamic memory chip select 0.
	LPC_EMC->DYNAMICRASCAS0 	= (2UL << 0) | (2UL << 8); // Selects the RAS and CAS latencies for dynamic memory chip select 0.	
	//LPC_EMC->DYNAMICRASCAS0 	= (3UL << 0) | (3UL << 8); // Selects the RAS and CAS latencies for dynamic memory chip select 0.	
	LPC_EMC->DYNAMICREADCONFIG	= EMC_COMMAND_DELAYED_STRATEGY;	 // Configures the dynamic memory read strategy.
	LPC_EMC->DYNAMICRP 		    = 3-1; // Selects the precharge command period
	LPC_EMC->DYNAMICRAS 		= 7-1; // Selects the active to precharge command period
	LPC_EMC->DYNAMICSREX 		=11-1; // Selects the self-refresh exit time
	LPC_EMC->DYNAMICAPR 		= 2-1; // Selects the last-data-out to active command time
	LPC_EMC->DYNAMICDAL 		= 5-1; // = tWR + tRP Selects the data-in to active command time.
	LPC_EMC->DYNAMICWR 		    = 2-1; // Selects the write recovery time
	LPC_EMC->DYNAMICRC 	   	    =10-1; // Selects the active to active command period
	LPC_EMC->DYNAMICRFC 		=10-1; // Selects the auto-refresh period
	LPC_EMC->DYNAMICXSR 		=11-1; // Selects the exit self-refresh to active command time
	LPC_EMC->DYNAMICRRD 		= 2-1; // Selects the active bank A to active bank B latency
	LPC_EMC->DYNAMICMRD 		= 2-1; // Selects the load mode register to active command time
	
	vDelay(100);
	LPC_EMC->DYNAMICCONTROL 	= EMC_CE_ENABLE | EMC_CS_ENABLE | EMC_INIT(EMC_NOP);
	DEBUGSTR("LPC_EMC-> setting #1 ok.\r\n");
	vDelay(100);
	DEBUGSTR("vDelay(100);\r\n");
	...
	LPC_EMC->DYNAMICREFRESH 	= 2; // Configures dynamic memory refresh operation
	DEBUGSTR("LPC_EMC-> setting #2 ok.\r\n");
	vDelay(100);
	DEBUGSTR("vDelay(100);\r\n");
	
	LPC_EMC->DYNAMICREFRESH 	= (795/16); // Configures dynamic memory refresh operation
	
	LPC_EMC->DYNAMICCONTROL 	= EMC_CE_ENABLE | EMC_CS_ENABLE | EMC_INIT(EMC_MODE);
	DEBUGSTR("LPC_EMC-> setting #3 ok.\r\n");
	// Write configuration data to SDRAM device
	if(u32DataBus == 0)   // 16-bit data bus, the EMC enforces a burst size 8
	{
		*((volatile uint32_t *)(u32BaseAddr | ((3UL | (2UL << 4)) << (u32ColAddrBits + 2 + 1))));
	...
	*/
	...
	DEBUGSTR(" Write configuration data to SDRAM device...ok.\r\n");
#endif   // AOCI_BOARD
	
//#if (PLATFORM == NXP_EVAL_BOARD)	
...
//#if (PLATFORM == NXP_VALIDATION_BOARD)	
	...
	DEBUGSTR("Enable the buffers...ok.\r\n");
	...
	DEBUGSTR("Enable the Ext SRAM buffers...ok.\r\n");
#endif		
